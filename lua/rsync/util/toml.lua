...
-- reads a file and parses it as toml
TOML.parse_file = function(path, options)
    local exists = vim.fn.filereadable(path)
    if exists ~= 1 then
        log.error(string.format('Config file not found: %s', path))
        return nil
    end

    local file = io.open(path, 'r')
    if file == nil then
        log.error(string.format('Config file could not be opened: %s', path))
        return nil
    end
    local toml_content = file:read('*all')
    file:close()

    local toml_table = TOML.parse(toml_content, options)
    return toml_table
end
